...
extension simd_float4x3: Value {
    public static func value(from object: Any) throws -> simd_float4x3 {
        if let data = object as? Extractable {
            let col0: simd_float3 = try data.value(for: keys.column0)
            let col1: simd_float3 = try data.value(for: keys.column1)
            let col2: simd_float3 = try data.value(for: keys.column2)
            let col3: simd_float3 = try data.value(for: keys.column3)
            
            return simd_float4x3(columns: (col0, col1, col2, col3))
        }
        else if let data = object as? IndexExtractable {
            let col0: simd_float3 = try data.value(for: indexes.column0)
            let col1: simd_float3 = try data.value(for: indexes.column1)
            let col2: simd_float3 = try data.value(for: indexes.column2)
            let col3: simd_float3 = try data.value(for: indexes.column3)
...
extension simd_float4x3: Serializable {
...
extension simd_float4x3: IndexSerializable {
