...
extension simd_double4x4: Value {
    public static func value(from object: Any) throws -> simd_double4x4 {
        if let data = object as? Extractable {
            let col0: simd_double4 = try data.value(for: keys.column0)
            let col1: simd_double4 = try data.value(for: keys.column1)
            let col2: simd_double4 = try data.value(for: keys.column2)
            let col3: simd_double4 = try data.value(for: keys.column3)
            ...
            let col2: simd_double4 = try data.value(for: indexes.column2)
            let col3: simd_double4 = try data.value(for: indexes.column3)
...
extension simd_double4x4: Serializable {
...
extension simd_double4x4: IndexSerializable {
