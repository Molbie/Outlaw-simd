...
public extension matrix_float4x3 {
...
extension matrix_float4x3: Value {
    public static func value(from object: Any) throws -> matrix_float4x3 {
        if let data = object as? Extractable {
            typealias keys = matrix_float4x3.ExtractableKeys
            
            let col0: vector_float3 = try data.value(for: keys.column0)
            let col1: vector_float3 = try data.value(for: keys.column1)
            let col2: vector_float3 = try data.value(for: keys.column2)
            let col3: vector_float3 = try data.value(for: keys.column3)
            
            return matrix_float4x3(columns: (col0, col1, col2, col3))
        }
        else if let data = object as? IndexExtractable {
            typealias indexes = matrix_float4x3.ExtractableIndexes
            
            let col0: vector_float3 = try data.value(for: indexes.column0)
            let col1: vector_float3 = try data.value(for: indexes.column1)
            let col2: vector_float3 = try data.value(for: indexes.column2)
            let col3: vector_float3 = try data.value(for: indexes.column3)
...
extension matrix_float4x3: Serializable {
...
extension matrix_float4x3: IndexSerializable {
