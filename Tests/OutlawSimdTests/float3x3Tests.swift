...
class float3x3Tests: XCTestCase {
    private typealias keys = simd_float3x3.keys
    private typealias subkeys = SIMD3<Float>.keys
    private typealias indexes = simd_float3x3.indexes
    private typealias subindexes = SIMD3<Float>.indexes
    ...
    func testExtractableValue() {
        ...
        let value: float3x3 = try! data.value(for: "value")
    ...
    func testIndexExtractableValue() {
        ...
        let value: float3x3 = try! data.value(for: 0)
    ...
    func testInvalidValue() {
        ...
        do {
            let _: float3x3 = try data.value(for: 0)
        }
        catch {
    ...
    func testSerializable() {
        let value = float3x3([float3(0, 10, 20),
                              float3(1, 11, 21),
                              float3(2, 12, 22)])
