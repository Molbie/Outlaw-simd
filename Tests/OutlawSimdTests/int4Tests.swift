...
class int4Tests: XCTestCase {
    private typealias keys = SIMD4<Int>.keys
    private typealias indexes = SIMD4<Int>.indexes
    ...
    func testExtractableValue() {
        ...
        let value: int4 = try! data.value(for: "value")
    ...
    func testIndexExtractableValue() {
        var rawData = [Int32](repeating: 0, count: 4)
        rawData[indexes.x] = 1
        rawData[indexes.y] = 2
        rawData[indexes.z] = 3
        rawData[indexes.w] = 4
        ...
        let value: int4 = try! data.value(for: 0)
    ...
    func testInvalidValue() {
        ...
        do {
            let _: int4 = try data.value(for: 0)
        }
        catch {
    ...
    func testSerializable() {
        let value = int4(x: 1, y: 2, z: 3, w: 4)
